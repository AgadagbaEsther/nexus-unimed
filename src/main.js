...
function setupResize() {
    ...
    const doResize = () => {
        ...
        renderer.setSize(window.innerWidth, window.innerHeight);
        lockMobileViewport();
        // Resizing the canvas clears its WebGL drawing buffer, and our
        // render-on-demand loop only renders when something flags it —
        // a plain resize doesn't. Without this, the model would stay
        // blank until the next drag/zoom (e.g. after opening/closing
        // DevTools, rotating the device, or toggling mobile emulation).
        requestRender();
    };
